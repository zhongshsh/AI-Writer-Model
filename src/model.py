...
class RWKV_TimeMix(nn.Module):
    def __init__(self, config, layer_id):
        ...
        with flow.no_grad():  # initial time_w curves for better convergence
            ww = flow.ones(config.n_head, config.ctx_len)
            curve = flow.tensor(
            ...
            for h in range(config.n_head):
                if h < config.n_head - 1:
                    ...
                    decay_speed = 0.0
                ww[h] = flow.exp(curve * decay_speed)
                # print('layer', layer_id, 'head', h, 'decay_speed', round(decay_speed, 4), ww[h][:5].numpy(), '...', ww[h][-5:].numpy())
        self.time_w = nn.Parameter(ww)

        self.time_alpha = nn.Parameter(flow.ones(self.n_head, 1, config.ctx_len))
        self.time_beta = nn.Parameter(flow.ones(self.n_head, config.ctx_len, 1))
    ...
    def forward(self, x):
        ...
        x = flow.cat([self.time_shift(x[:, :, : C // 2]), x[:, :, C // 2 :]], dim=-1)

        k = self.key(x)
        v = self.value(x)
        r = self.receptance(x)

        k = flow.clamp(k, max=30, min=-60)  # clamp extreme values. e^30 = 10^13
        k = flow.exp(k)
        sum_k = flow.cumsum(k, dim=1)

        kv = (k * v).view(B, T, self.n_head, self.head_size)

        wkv = (flow.einsum("htu,buhc->bthc", w, kv)).contiguous().view(B, T, -1)
...
class RotaryEmbedding(flow.nn.Module):
    def __init__(self, dim, base=10000):
        super().__init__()
        inv_freq = 1.0 / (base ** (flow.arange(0, dim, 2).float() / dim))
    ...
    def forward(self, x, seq_len=None):
        if seq_len != self.seq_len_cached:
            ...
            t = flow.arange(seq_len, device=x.device)
            freqs = flow.einsum("i,j->ij", t, self.inv_freq)
            emb = flow.cat((freqs, freqs), dim=-1).to(x.device)
...
class MHA_rotary(nn.Module):
    def __init__(self, config, layer_id, time_shift=False):
        ...
        self.register_buffer(
            "mask", flow.tril(flow.ones(config.ctx_len, config.ctx_len))
        )
    ...
    def forward(self, x):
        ...
        q = flow.cat((q, query_pass), dim=-1)
        k = flow.cat((k, key_pass), dim=-1)
...
class MHA_pro(nn.Module):
    def __init__(self, config, layer_id):
        ...
        self.time_w = nn.Parameter(flow.ones(self.n_head, config.ctx_len))
        self.time_alpha = nn.Parameter(flow.ones(self.n_head, 1, config.ctx_len))
        self.time_beta = nn.Parameter(flow.ones(self.n_head, config.ctx_len, 1))
        self.time_gamma = nn.Parameter(flow.ones(config.ctx_len, 1))
        self.register_buffer(
...
class RMSNorm(nn.Module):
    def __init__(self, d):
        super().__init__()
        self.dd = d ** (-1.0 / 2)
        self.weight = nn.Parameter(flow.ones(d))
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.register_buffer(
            "copy_mask", flow.tril(flow.ones(config.ctx_len, config.ctx_len))
        )
    ...
    def forward(self, idx, targets=None):
        ...
        if targets is not None:
            ...
            loss = nn.CrossEntropyLoss(reduction="none")(
                x.view(-1, x.size(-1)), targets.view(-1)
            )
