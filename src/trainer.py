...
class Trainer:
    def __init__(self, model, train_dataset, test_dataset, config):
        ...
        if flow.cuda.is_available():  # take over whatever gpus are on the system
            # self.device = flow.cuda.current_device()
            self.device = "cuda:0"
            # self.model = flow.nn.parallel.DistributedDataParallel(self.model).to(self.device)
    ...
    def train(self):
        ...
        def run_epoch(split):
                ...
                if is_train:  # backprop and update the parameters
                    ...
                    if config.lr_decay:  # decay the learning rate based on our progress
                        ...
                        if self.tokens < config.warmup_tokens:
                            ...
                            progress = float(
                                (self.tokens - config.warmup_tokens).numpy()
                            ) / float(
            ...
            logger.info(f"epoch {epoch+1} \t loss {loss_sum:.4f}")
        ...
        for epoch in range(config.max_epochs):
            ...
            if (
                ...
                flow.save(raw_model, self.config.epoch_save_path + str(epoch + 1))
